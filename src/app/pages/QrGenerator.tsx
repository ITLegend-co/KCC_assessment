...
export default function QrGenerator() {
  ...
  const [qrMode, setQrMode] = useState<'bib' | 'boulder'>('bib');
  ...
  return (
    <div className="kcc-page kcc-qr-page min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] md:p-6">
      <div className="mx-auto max-w-4xl">
        <div className="mb-6"><BackButton title="Generate" accent="QR Codes" /></div>
        <div className="kcc-panel rounded-xl bg-white p-4 shadow-lg sm:p-6 md:p-8">
          ...
          <div className="kcc-qr-tabs mb-5 grid grid-cols-2">
            <button type="button" onClick={() => setQrMode('bib')} aria-pressed={qrMode === 'bib'}>Student BIB</button>
            <button type="button" onClick={() => setQrMode('boulder')} aria-pressed={qrMode === 'boulder'}>Boulder</button>
          </div>

          <div className="kcc-qr-workspace">
            {qrMode === 'bib' && <section className="space-y-4">
            ...
            {qrMode === 'boulder' && <section className="space-y-4">
