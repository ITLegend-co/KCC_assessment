...
interface ArchiveRecord {
  ...
  studentAssessments?: Record<string, unknown>;
...
export function AssessmentArchives({ username }: { username: string }) {
  ...
  const snapshotCurrent = async () => {
    const [students, scores, studentAssessments, settings] = await Promise.all([
    ...
    return {
      ...
      scores: scores.val() || {},
      studentAssessments: studentAssessments.val() || {},
  ...
  const restoreArchive = async (archive: ArchiveRecord) => {
    ...
    try {
      await update(ref(database), {
        students: archive.students || null,
        scores: archive.scores || null,
        studentAssessments: archive.studentAssessments || null,
        settings: archive.settings || null,
      });
